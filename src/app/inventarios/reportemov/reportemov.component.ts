...
export class ReportemovComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.currentDate();
    this.reportemovForm.patchValue({FechaFrom: this.nowFrom , FechaTo: this.nowTo});

    this.inventariosService.currentLoading.subscribe(res => {
      this.loading = res;
    });

    this.inventariosService.currentConsultaReportemovSend.subscribe(res => {
      this.consulta = res;
    });

    this.inventariosService.currentMoneda.subscribe(res => {
      this.moneda = res;
    });
  }
...
/*
  ...
  onSubmit() {
    ...
    this.inventariosService.currentDataReportemov.subscribe(res => {
      this.reportemov = res;
  ...
  exportMovimientos.unshift({Fecha:"FECHA",
    Documento:"TIPO",
    Serie:"SERIE",
    Correlativo:"NÚMERO",
    Producto:"PRODUCTO",
