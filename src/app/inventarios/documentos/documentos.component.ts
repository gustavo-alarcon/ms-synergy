...
export class DocumentosComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.loginService.currentPermissions.subscribe(res => {
      ...
      this.cols = this.perms[0]['doc_t_correlativoInicial'] + this.perms[0]['doc_t_correlativoActual'];
      if(this.cols === 0){
        this.rows = 1;
      }else{
        this.rows = 2;
  ...
  filterData(ref: string) {
    this.documentosFiltrados = this.data_.filter( value => 
      value['Abrev'].startsWith(ref) || value['Nombre'].startsWith(ref) || value['Tipo'].startsWith(ref) || value['Naturaleza'].startsWith(ref) || value['Numtienda'].startsWith(ref) || value['Correlativo_inicial'].startsWith(ref) || value['Correlativo_actual'].startsWith(ref) || value['Uso'].startsWith(ref) || value['Detalle'].startsWith(ref)
    );
  }
  ...
  editAction( idx: number) {
    ...
    this.modData['Correlativo_inicial'] = this.documentosFiltrados[idx]['Correlativo_inicial'];
    this.modData['Correlativo_actual'] = this.documentosFiltrados[idx]['Correlativo_actual'];
