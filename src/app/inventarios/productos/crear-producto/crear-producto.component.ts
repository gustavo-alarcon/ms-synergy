...
export class CrearProductoComponent implements OnInit {
  ...
  calcAlarma() {
    this.stockA = this.crearProductoForm.value['Stocke'] * (1 + this.crearProductoForm.value['Offset_stocka']/100);
    this.crearProductoForm.patchValue({
      Stocka: this.stockA
    });
  }
  ...
  checkByNombre(target:string) {
    ...
    if (this.productos.filter(producto => producto['Nombre'] === target).length > 0) {
  ...
  checkByCodigo(target:string) {
    ...
    this.tempCodigos = this.productos.filter(producto => producto['Codigo'] === target);
  ...
  checkDuplicates(nombre:string, codigo:string, almacen:string) {
    ...
    this.productos.forEach(element => {
      if (element['Nombre'] === nombre && element['Codigo'] === codigo && element['Zona'] === almacen) {
        this.message = 'El producto: ('+ nombre + ', ' + codigo + ') ya existe en el almacen: ' + almacen;
        this.crearProd = false;
      }

      if (element['Nombre'] === nombre && element['Codigo'] === codigo && element['Zona'] !== almacen) {
        this.message = 'Se duplicará el producto: ('+ nombre + ', ' + codigo + ') en el almacen: ' + almacen;
      }

      if (element['Nombre'] === nombre && element['Codigo'] !== codigo && element['Zona'] === almacen) {
        this.message = 'No se puede asignar el código ('+codigo+') a un producto existente';
        this.crearProd = false;
      }
      ...
      if (element['Nombre'] !== nombre && element['Codigo'] === codigo && element['Zona'] === almacen) {
        this.message = 'No se puede asignar ('+nombre+') a un código existente';
        this.crearProd = false;
      }
